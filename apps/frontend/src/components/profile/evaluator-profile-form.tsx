...
export function EvaluatorProfileForm() {
  const { refreshProfile } = useAuth();
  const queryClient = useQueryClient();
  const [picture, setPicture] = useState<File | null>(null);

  const profileQuery = useQuery({
    queryKey: queryKeys.evaluator.profile(),
    queryFn: () =>
      evaluatorPageService.getProfile() as Promise<UserProfile | null>,
  });

  const { data, isError, error, refetch, isLoading } = profileQuery;
  ...
  useEffect(() => {
    if (data) {
      reset({
        ...
        department: (data.department as FormData["department"]) ?? "CS",
