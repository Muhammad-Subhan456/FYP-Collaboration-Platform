...
export default function StudentTeamPage() {
  ...
  const isLeader =
    !!teamQuery.data && !!user && teamQuery.data.leaderId === user.userId;

  const requestsQuery = useQuery({
    queryKey: ["team", "join-requests"],
    queryFn: teamService.getJoinRequests,
    enabled: isLeader,
    retry: false,
  });
  ...
  const browseQuery = useQuery({
    ...
    queryFn: () =>
      activeSearch
        ? teamService.searchTeams(activeSearch)
  ...
  const requestIds = requestsQuery.data?.map((r) => r.authUserId) ?? [];
  const profileIds = teamQuery.data
    ? [...new Set([teamQuery.data.leaderId, ...memberIds, ...requestIds])]
    : [];
  ...
  if (teamQuery.isError) {
    return (
      <ErrorState
        message={getErrorMessage(teamQuery.error)}
      ...
      <TabsContent value="browse">
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            ) : (browseQuery.data?.length ?? 0) > 0 ? (
              <div className="grid gap-3 sm:grid-cols-2">
                {browseQuery.data?.map((t) => (
                  <div
                    key={t.id}
