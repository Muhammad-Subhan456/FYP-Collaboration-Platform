...
export default function StudentDashboardPage() {
  const dashboardQuery = useQuery({
    ...
    queryFn: dashboardService.getStudent,
  });

  const membersQuery = useQuery({
    queryKey: ["team", "members"],
    queryFn: teamService.getMyTeamMembers,
    enabled: !!dashboardQuery.data?.team,
  });
  ...
  const supervisorId = dashboardQuery.data?.proposal?.assignedSupervisorId;
  ...
  const nextDeliverable = [...deliverables]
    .filter((d: { dueDate?: string }) => d.dueDate)
    .sort(
  ...
  const nextEvaluation = [...evaluations]
    .filter((e: { evaluation?: { date?: string } }) => e.evaluation?.date)
    .sort(
      (
        a: { evaluation: { date: string } },
        b: { evaluation: { date: string } },
      ) =>
  ...
  const latestGlobalAnnouncement = globalAnnouncementsQuery.data?.[0];
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            {deliverables.length > 0 ? (
              deliverables.slice(0, 5).map((d: { id: string; title: string; dueDate: string }) => (
                <div
                  key={d.id}
          ...
          <CardContent className="space-y-3">
            {evaluations.length > 0 ? (
              evaluations.slice(0, 5).map((item: {
                id: string;
                evaluation: { title: string; date: string; venue?: string };
              }) => (
        ...
        <GlobalAnnouncementsCard
          announcements={globalAnnouncementsQuery.data}
        />
        ...
        <RecentActivityFeed className="lg:col-span-2" />
      </div>
    </div>
  );
}
