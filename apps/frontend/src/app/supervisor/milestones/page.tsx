...
import type { Milestone, MilestoneStatus, Task, TaskStatus } from "@/types/student";
...
function MilestoneTasks({
  ...
  const tasksQuery = useQuery({
    ...
    queryFn: () => supervisorService.getMilestoneTasks(milestoneId),
  });

  const profilesQuery = useProfilesLookup(teamMemberIds);
            ...
            >
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  Assigned to{" "}
                  {getDisplayName(profilesQuery.data, task.assignedTo)}
                  {task.dueDate
                ...
                <SelectContent>
                  {teamMemberIds.map((id) => (
                    <SelectItem key={id} value={id}>
                      {getDisplayName(profilesQuery.data, id)}
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
...
export default function SupervisorMilestonesPage() {
  ...
  const membersQuery = useQuery({
    queryKey: ["team", selectedProposal?.teamId, "members"],
    queryFn: () => teamService.getTeamMembers(selectedProposal!.teamId),
    enabled: !!selectedProposal?.teamId,
  });

  const milestonesQuery = useQuery({
    queryKey: ["milestones", effectiveProposalId],
    queryFn: () => supervisorService.getMilestones(effectiveProposalId),
    enabled: !!effectiveProposalId,
  });
  ...
  const teamMemberIds =
    membersQuery.data?.map((m) => m.authUserId) ?? [];
